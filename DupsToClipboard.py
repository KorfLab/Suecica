...
i_fn = command_line()
dups = []
pattern = r"^(\S+)\t(\d+)\t(\d+)\t\d+\t(\d+)\t(\S+)\t(\d+)$"
recomp = re.compile(pattern)
with open(i_fn) as infile:
	for line in infile:
		match = recomp.match(line)
		if match:
			(chr, start, end, state, cov, transposons) = match.groups()
			if state == "2":
				outline = str(chr) + ":" + str(start) + "-" + str(end) + "\t(" + str(cov) + ")\t(" + str(transposons) + " transposons)"
				if int(transposons) == 1: outline = outline[:-2] + ")"
				dups.append(outline)
