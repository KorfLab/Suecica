...
def command_line():
    parser = argparse.ArgumentParser(description="comp_dup_gene_rank_to_random_list.py establishes whether or not a list of genes believed to be duplicated, on average, have higher expression rank in Thalyrata-mapped RNA-Seq data when compared to an equally sized list of randomly selected genes.")
    #parser.add_argument('-i', default="Data/RNA-Seq/AsLeaf_vs_ColLeaf/mRNA_Expression_0_hr_Technicals_Combined.tsv", type=str, help='Input file containing mRNA expression data for A. suecica', metavar='ExpressionFile')
...
def rank_data(gene_count_dict):
    ranked_genes = {}
    exp_sorted = set(sorted(gene_count_dict.values()))
    values_to_genes = {v: [] for v in exp_sorted}
...
# Rank each gene's RPKM expression value
ranked_genes = rank_data(gene_count_dict)

# Sum up A. suecica duplicated gene ranks
sue_dup_sum = sum([c for gene, c in ranked_genes.items() if gene in dup_list])

# Create sampling gene dictionary that does not contain duplicated genes
sample_dict = {gene: c for gene, c in ranked_genes.items() if gene not in dup_list}

# Generate a random list of genes for loop_count times, sum the ranks for this random list, and compare
...
print("The duplicated list of genes' ranked RPKM value was greater than an equally sized, random list of genes for "
