...
# 2) Finds genic nucleotide positions within supposedly duplicated regions that have a 50/50 reference/one-nucleotide SNP
...
def Command_line():
	...
	#parser.add_argument('-o', default="Output/Sue1Set5_AaFiltered_SUNGenes.txt", type=str, help='Output filepath for SUN results (Default="Output/Sue1Set5_AaFiltered_SUNGenes.txt")', metavar='OutFilepath')
	parser.add_argument('-o', default="Output/Sue1Set5_AaFiltered_SUNGenes_Random.txt", type=str, help='Output filepath for SUN results (Default="Output/Sue1Set5_AaFiltered_SUNGenes.txt")', metavar='OutFilepath')
	...
	return(pileup_file, dup_file, gff_file, use_random, outpath)
	
pileup_file, dup_file, gff_file, use_random, sun_filename = Command_line()
...
with open(sun_filename[:-4] + "_SUNs.txt", 'w') as sue_sun_file: # Debugging file
...
with open(sun_filename, 'w') as outfile:
	line = "Gene\tChr\tStart_pos\tEnd_pos\tSUN_Count\tSUN_Avg_Prob\n"
	outfile.write(line)
	for entry in sorted(gene_dict.items(), key = lambda x: (x[1][3], x[0]), reverse=True):
